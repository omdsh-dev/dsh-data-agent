	...
	factory: (require) => {
		...
		function saveConnection(connection, storage = defaultStorage()) {
			...
			try {
				...
				if (toWrite.credentialMode === "reference" || toWrite.passwordRef !== void 0) {
		...
		*/
		...
		async function performConnect(sessionId, body, signal) {
			return parseJsonResponse(await fetch("/plugins/data-agent/connect", {
				method: "POST",
		...
		function DataAgentWorkbench({ sessionId, useSessions, t }) {
			...
			const [credentialMode, setCredentialMode] = (0, react.useState)(initialSaved?.credentialMode === "reference" || initialSaved?.passwordRef !== void 0 ? "reference" : "password");
			const [credentialStatus, setCredentialStatus] = (0, react.useState)();
			const [rememberPassword, setRememberPassword] = (0, react.useState)(initialSaved?.persistPassword === true);
			const [readonly, setReadonly] = (0, react.useState)(initialSaved?.readonly === true);
			...
			const connect = async () => {
				...
				try {
					...
					if (result.ok) {
						...
						setCredentialStatus(result.summary?.credential);
						setConnectModalOpen(false);
						const schemasBody = await parseJsonResponse(await fetch(`/plugins/data-agent/schemas?sessionId=${encodeURIComponent(sessionId)}`));
			...
			const disconnect = async () => {
				...
				setError(null);
				try {
					await parseJsonResponse(await fetch("/plugins/data-agent/disconnect", {
						method: "POST",
						headers: { "content-type": "application/json" },
			...
			const toggleSchema = async (schema) => {
				...
				setTables([]);
				try {
					const result = await parseJsonResponse(await fetch(`/plugins/data-agent/tables?sessionId=${encodeURIComponent(sessionId)}&schema=${encodeURIComponent(schema)}`));
			...
			const selectTable = async (table) => {
				...
				try {
					const result = await parseJsonResponse(await fetch(`/plugins/data-agent/describe?${params.toString()}`));
			...
			const runSql = async () => {
				...
				setSqlResult(null);
				try {
					const result = await parseJsonResponse(await fetch("/plugins/data-agent/query", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({
							sessionId,
