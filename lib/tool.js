import { i as DEFAULT_MAX_RESULT_CHARS, l as classifyStatement, o as DEFAULT_QUERY_TIMEOUT_MS, u as clientsSchema } from "./defaults-DSPWeO3b.js";
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		name: "sqlcmd",
		description: `在已连接的数据库上执行 SQL 或客户端命令（如 SHOW TABLES、DESCRIBE users、SELECT * FROM orders LIMIT ${resolved.maxRows}）。需要先在「数据库」标签页连接数据库；SQL 经 stdin 传给客户端（mysql/psql/sqlite3/sqlplus/beeline/impala-shell/clickhouse-client/sqlcmd 等），无 shell 层。结果包含 exitCode 与 stdout/stderr 文本。`,
		parameters: { sql: {
			type: "string",
			required: true,
