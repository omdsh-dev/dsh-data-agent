...
function credentialEnv(type, connection) {
	...
	if (password === void 0) return type === "oracle" ? { NLS_LANG: "AMERICAN_AMERICA.AL32UTF8" } : {};
	switch (type) {
		case "mysql":
		...
		case "oracle": return { NLS_LANG: "AMERICAN_AMERICA.AL32UTF8" };
		case "clickhouse":
		case "sqlite":
		case "hive":
		case "impala": return {};
...
function buildClientStdin(type, mode, prefix, sql) {
	...
	if (type === "oracle" && !/;\s*$|\/\s*$/.test(sql) && /^(select|with|insert|update|delete|merge|create|drop|alter|truncate|comment|grant|revoke|explain|call|declare|begin)\b/i.test(sql.trimStart())) return `${prefix}${sql};\n`;
	return `${prefix}${sql}\n`;
}
