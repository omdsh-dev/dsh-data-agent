...
function sanitizeIdentifier(type, identifier) {
	...
	switch (type) {
		...
		case "impala":
		case "clickhouse":
		case "doris": return "`" + identifier.replace(/`/g, "``") + "`";
		case "postgres":
		case "oracle":
...
function connectionArgs(type, connection) {
	switch (type) {
		...
		case "doris": return [
			...
			connection.host ?? "127.0.0.1",
			"-P",
			String(connection.port ?? DEFAULT_PORTS.doris),
			"-u",
			connection.user ?? "root",
			"-D",
			connection.database
		...
		case "clickhouse": return [
			...
			connection.host ?? "127.0.0.1",
			"--port",
			String(connection.port ?? DEFAULT_PORTS.clickhouse),
			"--user",
		...
		case "sqlserver": return [
			"-S",
			`${connection.host ?? "127.0.0.1"},${connection.port ?? DEFAULT_PORTS.sqlserver}`,
			...connection.user !== void 0 && connection.user !== "" ? ["-U", connection.user] : ["-E"],
			"-d",
			connection.database
		];
		case "oracle":
...
function credentialEnv(type, connection) {
	...
	switch (type) {
		...
		case "clickhouse": return { CLICKHOUSE_PASSWORD: password };
...
function parseColumns(type, stdout) {
	...
	for (let index = start; index < lines.length; index += 1) {
		...
		switch (type) {
			...
			case "hive":
			case "impala":
			case "clickhouse": nullable = void 0;
		}
		columns.push({
			name,
			type: columnType,
